...
public partial class CreatePages : System.Web.UI.UserControl
{
    ...
    public string LoadPage()
    {
        ...
        OleDbCommand CMD = new OleDbCommand("SELECT Data_Path FROM MenuItem Where ItemID = " + cb.Value.ToString(), Con);
        try
        {
            Con.Open();
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        if (Path == string.Empty)
        {
            ...
            Path = String.Format("../ItemData/Data{0}{1}{2}{3}.Dat", cb.Value, DateTime.Now.Hour, DateTime.Now.Minute, DateTime.Now.Second);
        }
        try
        {
            CMD.CommandText = String.Format("Update MenuItem Set Data_Path = '{0}' Where ItemID = {1}", Path, cb.Value);
            Con.Open();
            CMD.ExecuteNonQuery();
            Path = MapPath(Path);
            //if (File.Exists(Path))
