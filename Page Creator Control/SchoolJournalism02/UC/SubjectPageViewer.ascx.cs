...
public partial class SubjectPageViewer : System.Web.UI.UserControl
{
    ...
    public string LoadContain()
    {
        ...
        {
            OleDbCommand cmd = new OleDbCommand(String.Format("SELECT Data_Path FROM school WHERE (aboutid = {0})", Request.QueryString["id"]), Con);
            try
            {
                Con.Open();
