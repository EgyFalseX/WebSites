...
public partial class CreatePages : System.Web.UI.UserControl
{
    ...
    private DataTable LoadData()
    {
        ...
        using (OleDbDataAdapter da = new OleDbDataAdapter("SELECT ItemID, ItemName, Show_Hide, Data_Path FROM MenuItem where  ItemID=?", constr))
        {
            da.Fill(dt);
        }
        return dt;
    ...
    public string LoadPage()
    {
        ...
        OleDbCommand CMD = new OleDbCommand("SELECT Data_Path FROM MenuItem Where ItemID = " + Request.QueryString["id"].ToString(), Con);
        try
        {
            Con.Open();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Request.QueryString["id"] != null)
        {
            ...
            Response.Redirect("../centerEditor.aspx");
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            CMD.CommandText = String.Format("Update MenuItem Set Data_Path = '{0}' Where ItemID = {1}", Path, Request.QueryString["id"]);
            Con.Open();
            CMD.ExecuteNonQuery();
            Path = MapPath(Path);
            //if (File.Exists(Path))
