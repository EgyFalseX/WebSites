...
s.cpShowPopup = undefined;
                ...
                <Columns>
                    ...
                    <dx:GridViewDataTextColumn Caption="كلمة المرور" FieldName="UserPass" 
                        VisibleIndex="2">
                    </dx:GridViewDataTextColumn>
                    <dx:GridViewDataComboBoxColumn Caption="النقابة" FieldName="SyndicateId" 
                        VisibleIndex="3">
                        <PropertiesComboBox DataSourceID="DSCDSyndicate" TextField="Syndicate" 
                            ValueField="SyndicateId" ValueType="System.Int32">
                            <Columns>
                                <dx:ListBoxColumn Caption="النقابة" FieldName="Syndicate" />
                            </Columns>
                        </PropertiesComboBox>
            ...
            <asp:SqlDataSource ID="DSData" runat="server" 
                ...
                DeleteCommand="DELETE FROM [Users] WHERE [UserID] = @UserID" InsertCommand="INSERT INTO [Users] ([UserID], [UserName], [UserPass], [SyndicateId], [ISAdmin]) 
VALUES 
((select ISNULL(MAX(UserID) + 1, 1) from Users), @UserName, @UserPass, @SyndicateId, @ISAdmin)" 
                ProviderName="<%$ ConnectionStrings:ETSDATAConnectionString.ProviderName %>" 
                SelectCommand="SELECT [UserID], [UserName], [UserPass], [SyndicateId], [ISAdmin] FROM [Users]" 
                
                UpdateCommand="UPDATE [Users] SET [UserName] = @UserName, [UserPass] = @UserPass, [SyndicateId] = @SyndicateId, [ISAdmin] = @ISAdmin WHERE [UserID] = @UserID">
                <DeleteParameters>
