<%@ Control Language="C#" AutoEventWireup="true" CodeFile="TBLSyndicateData.ascx.cs" Inherits="TBLSyndicateData" %>
...
s.cpShowPopup = undefined;
                ...
                <Columns>
                    ...
                    <dx:GridViewDataTextColumn Caption="تليفون1" 
                        VisibleIndex="3" FieldName="sphone1">
                    </dx:GridViewDataTextColumn>
                    <dx:GridViewDataTextColumn Caption="موبيل" FieldName="smobil" VisibleIndex="5">
                    </dx:GridViewDataTextColumn>
                    <dx:GridViewDataTextColumn Caption="فاكس" FieldName="sfax" VisibleIndex="6">
                    </dx:GridViewDataTextColumn>
                    <dx:GridViewDataTextColumn Caption="ايميل" FieldName="smail" VisibleIndex="7">
                    </dx:GridViewDataTextColumn>
...
{2}" notallowedfileextensionerrortext="امتداد الملف غير مسموح به">
                    ...
                    <dx:GridViewDataComboBoxColumn Caption="اسم النقابة" FieldName="SyndicateId" 
                        VisibleIndex="1">
                        <PropertiesComboBox DataSourceID="DSCDSyndicate" TextField="Syndicate" 
                            ValueField="SyndicateId" ValueType="System.Int32">
                        </PropertiesComboBox>
            ...
            <asp:SqlDataSource ID="DSData" runat="server" 
                ...
                DeleteCommand="SELECT [SyndicateId], [saddress], [sphone1], [sphone2], [smobil], [sfax], [smail], [slogo] FROM [TBLSyndicateData]" InsertCommand="SELECT [SyndicateId], [saddress], [sphone1], [sphone2], [smobil], [sfax], [smail], [slogo] FROM [TBLSyndicateData]" 
                ProviderName="<%$ ConnectionStrings:ETSDATAConnectionString.ProviderName %>" 
                SelectCommand="SELECT [SyndicateId], [saddress], [sphone1], [sphone2], [smobil], [sfax], [smail], [slogo] FROM [TBLSyndicateData]
WHERE        EXISTS
                             (SELECT        SyndicateId
                                FROM            Users
                                WHERE        (UserID = @UserID) AND (SyndicateId = TBLSyndicateData.SyndicateId)) OR
                         EXISTS
                             (SELECT        ISAdmin
                                FROM            Users
                ...
                UpdateCommand="SELECT [SyndicateId], [saddress], [sphone1], [sphone2], [smobil], [sfax], [smail], [slogo] FROM [TBLSyndicateData]">
                <SelectParameters>
                    <asp:SessionParameter Name="UserID" SessionField="SyndID" />
                </SelectParameters>
            </asp:SqlDataSource>
            <asp:SqlDataSource ID="DSCDSyndicate" runat="server" 
                ConnectionString="<%$ ConnectionStrings:ETSDATAConnectionString %>" 
                ProviderName="<%$ ConnectionStrings:ETSDATAConnectionString.ProviderName %>" 
                SelectCommand="SELECT [SyndicateId], [Syndicate] FROM [CDSyndicate]
WHERE        EXISTS
                             (SELECT        SyndicateId
                                FROM            Users
                                WHERE        (UserID = @UserID) AND (SyndicateId = CDSyndicate.SyndicateId)) OR
                         EXISTS
                             (SELECT        ISAdmin
                                FROM            Users
                                WHERE        (UserID = @UserID) AND (ISAdmin = 'True'))">
