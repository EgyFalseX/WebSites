<%@ Control Language="C#" AutoEventWireup="true" CodeFile="EditUserMessagesLow.ascx.cs" Inherits="EditUserMessagesLow" %>
...
<SettingsText GroupPanel="اسحب عمود ثم افلته هنا لصناعه مجموعه" ConfirmDelete="هل انت متأكد؟" 
            ...
            <asp:AccessDataSource ID="DSData1" runat="server" DataFile="../App_Data/contact.mdb" 
                ...
                FROM contact_ask
WHERE adminclose = false AND
Exists (SELECT LagnaId FROM UserAppilty WHERE LagnaId = contact_ask.LagnaId AND EdaraId = contact_ask.EdaraId AND id = ?)
...
WHERE [addno] = ?">
                ...
                <SelectParameters>
                    <asp:SessionParameter Name="?" SessionField="SyndConID" />
                </SelectParameters>
                <UpdateParameters>
            ...
            <asp:AccessDataSource ID="DSData2" runat="server" DataFile="../App_Data/contact.mdb" 
                SelectCommand="SELECT addno, LagnaId, EdaraId, coname, massage, tel, mail, condate, agreeshow, adminagree, replydate, LagnaIdr, EdaraIdr, adminclose, condate, reply, (SELECT EdaraName FROM CDEdara WHERE EdaraId = contact_suggest.EdaraId) AS EdaraName, (SELECT LagnaName FROM CDLagna WHERE LagnaId = contact_suggest.LagnaId) AS LagnaName 
FROM contact_suggest WHERE adminclose = false AND
EXISTS (SELECT LagnaId FROM UserAppilty WHERE LagnaId = contact_suggest.LagnaId AND EdaraId = contact_suggest.EdaraId AND id = ?) 
...
mail = ? WHERE [addno] = ?">
            ...
            <asp:AccessDataSource ID="DSData3" runat="server" DataFile="../App_Data/contact.mdb" 
                ...
                FROM contact_shkwa
WHERE adminclose = false AND
Exists (SELECT LagnaId FROM UserAppilty WHERE LagnaId = contact_shkwa.LagnaId AND EdaraId = contact_shkwa.EdaraId AND id = ?)
...
mail = ? WHERE [addno] = ?">
            ...
            <asp:AccessDataSource ID="DSData4" runat="server" DataFile="../App_Data/contact.mdb" 
                ...
                FROM contact_thanks
WHERE adminclose = false AND
Exists (SELECT LagnaId FROM UserAppilty WHERE LagnaId = contact_thanks.LagnaId AND EdaraId = contact_thanks.EdaraId AND id = ?)
...
mail = ? WHERE [addno] = ?">
            ...
            <asp:AccessDataSource ID="DSData1User" runat="server" DataFile="../App_Data/contact.mdb" 
                ...
                (SELECT LagnaName FROM CDLagna WHERE LagnaId = contact_ask.LagnaId) AS LagnaName 
                FROM contact_ask
WHERE adminclose = false AND LagnaId = ? AND EdaraId = ?  AND
Exists (SELECT LagnaId FROM UserAppilty WHERE LagnaId = contact_ask.LagnaId AND EdaraId = contact_ask.EdaraId AND id = ?)
ORDER BY condate DESC" DeleteCommand="DELETE FROM [contact_ask] WHERE [addno] = ?" 
...
mail = ? WHERE [addno] = ?">
                ...
                <SelectParameters>
                    <asp:SessionParameter Name="LagnaId" SessionField="LagnaId" />
                    <asp:SessionParameter Name="EdaraId" SessionField="EdaraId" />
                    <asp:SessionParameter Name="SyndConID" SessionField="SyndConID" />
                </SelectParameters>
                <UpdateParameters>
                    <asp:SessionParameter Name="LagnaId" SessionField="LagnaId" />
                    <asp:SessionParameter Name="EdaraId" SessionField="EdaraId" />
