...
s.cpShowPopup = undefined;
            ...
            <asp:SqlDataSource ID="DSData" runat="server" 
                ConnectionString="<%$ ConnectionStrings:ETSTConnectionString %>" SelectCommand="SELECT [MemberId], [MemberName], [Bdate], [NID], [NIDEndDate], [Street], [HomeNo], [FlatNo], [Area], [City], [Mobile], [HomeTel], [WorkTel], [EMail], [KidNO], [ReceiptNo], MemberImage, 
...
(SELECT Gender FROM CDGender WHERE GenderId = TBLMembers.GenderId) AS Gender,
(SELECT MaritalStatus FROM CDMaritalStatus WHERE MaritalStatusId = TBLMembers.MaritalStatusId) AS MaritalStatus,
(SELECT IdType FROM CDIdType WHERE IdTypeId = TBLMembers.IdTypeId) AS IdType,
(SELECT GOV FROM CDGov WHERE GovId = TBLMembers.GovId) AS GOV,
(SELECT ContactType FROM CDContactType WHERE ContactTypeID = TBLMembers.ContactTypeID) AS ContactType,
