<%@ Control Language="C#" AutoEventWireup="true" CodeFile="DescionViewer.ascx.cs" Inherits="DescionViewer" %>
    ...
    <tr>
        <td>
            <dx:ASPxGridView ID="GVEditor" runat="server" AutoGenerateColumns="False" 
                ...
                onrowinserted="GVEditor_RowInserted" Width="650px" Font-Bold="True">
...
s.cpShowPopup = undefined;
                ...
                <Columns>
                    <dx:GridViewDataMemoColumn Caption="القرارات" FieldName="desc" VisibleIndex="6">
                    </dx:GridViewDataMemoColumn>
                    <dx:GridViewDataTextColumn Caption="اللجنه" FieldName="LagnaName" 
                        VisibleIndex="0">
                    </dx:GridViewDataTextColumn>
                    <dx:GridViewDataTextColumn Caption="اللجنه النوعيه" FieldName="LagnaNoaeaName" 
                        VisibleIndex="1">
                    </dx:GridViewDataTextColumn>
            ...
            <asp:AccessDataSource ID="DSData" runat="server" DataFile="../App_Data/Synricate.mdb" 
                SelectCommand="SELECT [MId], [desc], [id],
(SELECT LagnaName FROM CDLagna WHERE LagnaId = (SELECT LagnaId FROM TBLMetting WHERE MId =TBLmdescion.MId) ) AS LagnaName,
(SELECT LagnaNoaeaName FROM CDLagnaNoaea WHERE LagnaNoaeaId = (SELECT LagnaNoaeaId FROM TBLMetting WHERE MId =TBLmdescion.MId) ) AS LagnaNoaeaName,
(SELECT MDate FROM TBLMetting WHERE MId = TBLmdescion.MId) AS MDate 
 FROM [TBLmdescion]">
            </asp:AccessDataSource>
            <asp:AccessDataSource ID="DSTBLMetting" runat="server" DataFile="../App_Data/Synricate.mdb" 
                SelectCommand="SELECT [MId], 
[MDate] &amp; ' - ' &amp;
(SELECT LagnaName FROM CDLagna WHERE LagnaId = TBLMetting.LagnaId) AS details
FROM [TBLMetting]">
            </asp:AccessDataSource>
        </td>
    </tr>
