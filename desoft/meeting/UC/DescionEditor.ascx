<%@ Control Language="C#" AutoEventWireup="true" CodeFile="DescionEditor.ascx.cs" Inherits="DescionEditor" %>
...
s.cpShowPopup = undefined;
                ...
                <Columns>
                    ...
                    <dx:GridViewDataComboBoxColumn Caption="الاجتماع" FieldName="MId" 
                        VisibleIndex="1">
                        <PropertiesComboBox DataSourceID="DSTBLMetting" TextField="details" 
                            ValueField="MId" ValueType="System.Int32">
                        </PropertiesComboBox>
                    </dx:GridViewDataComboBoxColumn>
                    <dx:GridViewDataMemoColumn Caption="القرارات" FieldName="desc" VisibleIndex="3">
            ...
            <asp:AccessDataSource ID="DSData" runat="server" DataFile="../App_Data/Synricate.mdb" 
                DeleteCommand="DELETE FROM [TBLmdescion] WHERE id = ?" 
                InsertCommand="INSERT INTO [TBLmdescion] ([MId], [desc]) VALUES (?, ?)" 
                SelectCommand="SELECT [MId], [desc], [id] FROM [TBLmdescion]
WHERE EXISTS (SELECT id FROM UserMeeting WHERE 
 (LagnaId = (SELECT LagnaId FROM TBLMetting WHERE MId = TBLmdescion.MId)) AND (id = ?))" 
                
                
                UpdateCommand="UPDATE [TBLmdescion] SET [desc] = ?, [MId] = ? WHERE id = ?">
                <DeleteParameters>
...
WHERE (LagnaId = TBLMetting.LagnaId) AND (id = ?))">
                <SelectParameters>
                    <asp:SessionParameter Name="?" SessionField="meetingID" />
                </SelectParameters>
            </asp:AccessDataSource>
