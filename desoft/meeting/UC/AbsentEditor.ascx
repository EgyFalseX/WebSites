<%@ Control Language="C#" AutoEventWireup="true" CodeFile="AbsentEditor.ascx.cs" Inherits="AbsentEditor" %>
...
s.cpShowPopup = undefined;
            ...
            <asp:AccessDataSource ID="DSData" runat="server" DataFile="../App_Data/Synricate.mdb" 
                DeleteCommand="DELETE FROM [TBLmAbsent] WHERE id = ?" 
                InsertCommand="INSERT INTO [TBLmAbsent] ([MId], [MemberId], [Reson]) VALUES (?, ?, ?)" 
                SelectCommand="SELECT MId, MemberId, Reson, id FROM TBLmAbsent
 WHERE EXISTS (SELECT MId, MDate, LagnaId, counthodor, countabsent, filepath FROM TBLMetting WHERE (MId = TBLmAbsent.MId) AND EXISTS (SELECT id FROM UserMeeting WHERE (LagnaId = TBLMetting.LagnaId) AND (id = ?)))" 
                ...
                UpdateCommand="UPDATE [TBLmAbsent] SET [Reson] = ?, [MId] = ?, [MemberId] = ? WHERE id = ?">
                <DeleteParameters>
