<%@ Control Language="C#" AutoEventWireup="true" CodeFile="BookingEditor.ascx.cs" Inherits="BookingEditor" %>
    ...
    <tr>
        <td>
            <dx:ASPxGridView ID="GVEditor" runat="server" AutoGenerateColumns="False" 
                ...
                KeyFieldName="bookingid" RightToLeft="True" 
...
s.cpShowPopup = undefined;
                ...
                <Columns>
                    ...
                    <dx:GridViewDataComboBoxColumn Caption="القاعه" FieldName="hallId" 
                        VisibleIndex="1">
                        <PropertiesComboBox DataSourceID="DSCDHall" TextField="hallName" 
                            ValueField="hallId" ValueType="System.Int32">
                            <Columns>
                                <dx:ListBoxColumn Caption="اختر اسم" FieldName="hallName" />
                            </Columns>
                            <Style HorizontalAlign="Center">
                            </Style>
                        </PropertiesComboBox>
                    </dx:GridViewDataComboBoxColumn>
                    <dx:GridViewDataTextColumn Caption="اسم صاحب الحجز" FieldName="bookingname" 
                        VisibleIndex="7">
                    </dx:GridViewDataTextColumn>
                    <dx:GridViewDataTextColumn Caption="تليفون" FieldName="bookingtel" 
                        VisibleIndex="9">
                    </dx:GridViewDataTextColumn>
                    <dx:GridViewDataComboBoxColumn Caption="الفترة" FieldName="fatraid" 
                        VisibleIndex="3">
                        <PropertiesComboBox DataSourceID="DSCDFatra" TextField="fatra" 
                            ValueField="fatraid" ValueType="System.Int32">
                            <Columns>
                                <dx:ListBoxColumn Caption="اختر اسم" FieldName="fatraid" />
                            </Columns>
                        </PropertiesComboBox>
                    </dx:GridViewDataComboBoxColumn>
                    <dx:GridViewDataDateColumn Caption="تاريخ الحجز" FieldName="bookingDate" 
                        VisibleIndex="2">
            ...
            <asp:AccessDataSource ID="DSData" runat="server" DataFile="../App_Data/Synricate.mdb" 
                DeleteCommand="DELETE FROM [tblbooking] WHERE bookingid = ?" 
                InsertCommand="INSERT INTO [tblbooking] ([hallId], [fatraid], [bookingDate], [bookingname], [bookingtel], [datein]) VALUES (@hallId, @fatraid, @bookingDate, @bookingname, @bookingtel, NOW())" 
                SelectCommand="SELECT [bookingid], [hallId], [fatraid], [bookingDate], [bookingname], [bookingtel], [datein] FROM [tblbooking]
WHERE EXISTS (SELECT id FROM UserMeeting WHERE 
 (LagnaId = (SELECT LagnaId FROM CDHall WHERE hallId = tblbooking.hallId)) AND (id = ?))" 
                ...
                UpdateCommand="UPDATE [tblbooking] SET [bookingname] = ?, [bookingtel] = ?, [datein] = NOW(), hallId = ?, fatraid = ?, bookingDate = ? WHERE 
bookingid = ?">
                <DeleteParameters>
                ...
                <InsertParameters>
                    ...
                    <asp:Parameter Name="bookingname" Type="String" />
                    <asp:Parameter Name="bookingtel" Type="String" />
                </InsertParameters>
                <SelectParameters>
            ...
            <asp:AccessDataSource ID="DSCDHall" runat="server" DataFile="../App_Data/Synricate.mdb"
                
                SelectCommand="SELECT [hallId], [hallName] FROM [CDHall]
WHERE EXISTS (SELECT id FROM UserMeeting WHERE (LagnaId = CDHall.LagnaId) AND (id = ?))">
                <SelectParameters>
                    <asp:SessionParameter Name="?" SessionField="meetingID" />
                </SelectParameters>
            </asp:AccessDataSource>
            <asp:AccessDataSource ID="DSCDFatra" runat="server" DataFile="../App_Data/Synricate.mdb"
                
                SelectCommand="SELECT [fatraid], [fatra] FROM [CDFatra]">
            </asp:AccessDataSource>
        </td>
    </tr>
    </table>
