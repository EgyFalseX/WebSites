...
/// </summary>
public class ThumbnailSliderWebApplication_SliderClass
{
    private OleDbConnection conn;


    public  ThumbnailSliderWebApplication_SliderClass()
    {
        cmd = new OleDbCommand(new ThumbnailSliderWebApplication_DBConnectClass("Rotarordata.mdb").ToString());
        cmd.Connection = ThumbnailSliderWebApplication_DBConnectClass.conn;
    }
    ...
    public string UpdateDetails()
    {
        ...
        if (ThumbnailSliderWebApplication_DBConnectClass.connStatus == 0)
            ThumbnailSliderWebApplication_DBConnectClass.OpenConn();
        cmd.ExecuteNonQuery();
        ThumbnailSliderWebApplication_DBConnectClass.CloseConn() ;
        return "تم الحفظ بنجاح";

    }
    public DataTable GetSlidersData()
    {
        OleDbDataAdapter da = new OleDbDataAdapter("select * from RotatorData where viewpage=true", ThumbnailSliderWebApplication_DBConnectClass.conn);
    ...
    public DataTable GetSlidersDataByID()
    {
        ...
        OleDbDataAdapter da = new OleDbDataAdapter("select * from RotatorData where ID=@ID", ThumbnailSliderWebApplication_DBConnectClass.conn);
