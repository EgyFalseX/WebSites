...
public partial class CreatePages : System.Web.UI.UserControl
{
    ...
    #region -   Variables   - 
    string constr = "Provider=Microsoft.Jet.OLEDB.4.0; Data Source= " + HttpContext.Current.Server.MapPath("~/App_Data/Rotarordata.mdb");
    #endregion
    #region -    Functions    -
    private DataTable LoadData()
    {
        DataTable dt = new DataTable("FX2012");
        using (OleDbDataAdapter da = new OleDbDataAdapter("SELECT ID, Caption, Contain FROM RotatorData ORDER BY ID", constr))
        {
            da.Fill(dt);
        }
        return dt;
    ...
    public string LoadPage()
    {
        ...
        OleDbCommand CMD = new OleDbCommand("SELECT Contain FROM RotatorData Where ID = " + Request.QueryString["ID"].ToString(), Con);
        try
        {
            Con.Open();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("CPSlider.aspx");
        }
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            CMD.CommandText = String.Format("Update RotatorData Set Contain = '{0}' Where ID = {1}", Path, Request.QueryString["ID"]);
            Con.Open();
            CMD.ExecuteNonQuery();
            Path = MapPath(Path);
            //if (File.Exists(Path))
