<%@ Control Language="C#" AutoEventWireup="true" CodeFile="DataViewere.ascx.cs" Inherits="DataViewer" %>
<style type="text/css">
        .left
        {
            width: 31px;
    ...
    frame="border" >
        ...
        <button onclick="$find('<%=RadRotator1.ClientID%>').showNext(Telerik.Web.UI.RotatorScrollDirection.Left);"
                ...
                <td align="center" valign="top" class="style10" >
                    <asp:Label ID="Label2" runat="server" Font-Bold="True" Text='<%# Eval("Captione") %>'
...
<asp:AccessDataSource ID="DSData" runat="server" DataFile="App_Data/DataDB.mdb" SelectCommand="SELECT [ID], [Caption],[Captione], [Contain], [ImageLink], 'PageViewer.aspx?ID=' &amp; [ID] AS [Link] FROM [RotatorData] WHERE  [aboutid] = ? ORDER BY RotatorData.ID DESC;">
<SelectParameters>
