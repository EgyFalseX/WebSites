...
public partial class CreatePages : System.Web.UI.UserControl
{
    ...
    public string LoadPage()
    {
        ...
        OleDbDataAdapter da = new OleDbDataAdapter("SELECT [Link] FROM RotatorData Where [ID] = " + DDLItems.SelectedValue, constr);
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            cmd.CommandText = String.Format("Update RotatorData Set [Link] = '{0}' Where ID = {1}", Path, DDLItems.SelectedValue);
            con.Open();
            cmd.ExecuteNonQuery();
            Path = MapPath(Path);
