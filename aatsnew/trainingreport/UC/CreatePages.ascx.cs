...
public partial class CreatePages : System.Web.UI.UserControl
{
    ...
    private DataTable LoadData()
    {
        ...
        using (OleDbDataAdapter da = new OleDbDataAdapter("SELECT aboutid, address, smallinfo, smallinfoe FROM school where  aboutid=?", constr))
        {
            da.Fill(dt);
        }
        return dt;
    ...
    public string LoadPage()
    {
        ...
        OleDbCommand CMD = new OleDbCommand("SELECT smallinfoe FROM school Where aboutid = " + Request.QueryString["id"].ToString(), Con);
        try
        {
            Con.Open();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (Request.QueryString["id"] != null)
        {
            ...
            Response.Redirect("../subjectEditore.aspx");
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            CMD.CommandText = String.Format("Update school Set smallinfoe = '{0}' Where aboutid = {1}", Path, Request.QueryString["id"]);
            Con.Open();
            CMD.ExecuteNonQuery();
            Path = MapPath(Path);
            //if (File.Exists(Path))
